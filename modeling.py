...
def global_graph(PATH):
    ...
    while i <= len(data):
      ...
      while k <= num_Pcon:
        ...
        xcpt=BK.OVERLAY(G,i, Pcon, etcd_bool,num1, etcd, etcd2_bool,num2, etcd2, xcpt)
    ...
    while i <= site_num:
      ...
      while j< num_VNF:
        ...
        xcpt=BK.A_VNFs(G,site_num, VNF,elasticity,number_elasticity,monit,etcd_bool, etcd,xcpt) 
        ...
        while m <= num_VLcon:
         ...
         xcpt= BK.LA_con(G,VNF,elasticity,number_elasticity, VNF2, elasticity2_bool,num2, etcd_bool, elasticity3_bool,num3, etcd, xcpt)
         m=m+1
        ...
        while q <= num_VDcon:
         ...
         xcpt= BK.DA_con(G,i,site2,VNF,elasticity,number_elasticity, VNF2, elasticity2_bool,num2, etcd_bool, elasticity3_bool,num3, etcd,etcdM_bool, elasticity4_bool,num4, etcdM, xcpt)
...
def topology_graph_call(path):
   ...
   while i<= site_num:
      ...
      while j < num_VNF:
          ...
          else: Topo.add_node("{0}".format(str(name)))
...
def Service_Register_Nodes(G,path,liste,cpt):## liste of nodes in Register, and the first bono and last element in liste , cpt for register
